...
static bool broker_is_connected(void)
{
    ...
    sprintf(mqtt.sub_topic_self, "%s/%s/%s/set/#", mqtt.prefix, sysinfo.mac_str, sysinfo.dev_type);
...
{
    xTimerChangePeriod(state_msg_timer, (mqtt.interval * 1000) / portTICK_RATE_MS, 0);
    xTimerReset(state_msg_timer, 0);
}
...
{
    xSemaphoreGive(alarm_msg_semaphore);
}
...
void csro_mqtt_task(void *pvParameters)
{
    ...
    state_msg_timer = xTimerCreate("basic_msg_timer", (mqtt.interval * 1000)/portTICK_RATE_MS, pdTRUE, (void *)0, csro_mqtt_msg_trigger_state);
    ...
    while(true)
    {
        if (csro_system_get_wifi_status()) 
        {
            if (broker_is_connected()) 
            {
                ...
                if (xSemaphoreTake(state_msg_semaphore, 1) == pdTRUE) 
                {
