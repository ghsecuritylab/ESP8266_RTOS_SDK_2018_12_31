...
{
    if (index > ALARM_NUMBER - 1) { return; }
...
{
    for(size_t i = 0; i < ALARM_NUMBER; i++) { alarms[i].valid = false; }
    save_alarm_to_nvs();
}
...
void csro_datetime_set(char *time_str)
{
    ...
    if ((tim[0]<2018) || (tim[1]>12) || (tim[2]>31) || (tim[3]>23) || (tim[4]>59) || (tim[5]>59)) { return; }
    if (time_sync == false || tim[5] == 30) 
    {
        ...
        if (time_sync == false) { time_sync = true; datetime.time_on = datetime.time_now - datetime.time_run; }
...
static void second_timer_callback( TimerHandle_t xTimer )
{
    ...
    if (time_sync == true) 
    {
        ...
        if (datetime.time_info.tm_sec == 0) { datetime_check_alarm(); }
    }
}
